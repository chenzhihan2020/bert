...
def read_squad_examples(input_file, is_training):
  ...
  examples = []
  i_num_cnt = 0
  print("Read Squad Example.\n")
  for entry in input_data:
    '''  
    i_num_cnt += 1
    if i_num_cnt > 10: 
        print("The number of doc is %d\n" % i_num_cnt)
        break
    '''    
...
##
    ...
    for (doc_span_index, doc_span) in enumerate(doc_spans):
      ...
      if is_training and not example.is_impossible:
        ...
        out_of_span = True
##find out whether a span contains any of answers
        start_position=[]
        end_position=[]
...
##softmax
  def softmax(x):
    ...
    sum_x=sum(exp_x)
    softmax_x = [i / sum_x  for i in exp_x]
    return softmax_x  
##
  ...
  ##Write the outputPrediction to file
  all_outputPredictions = collections.OrderedDict()
  ##
  for (example_index, example) in enumerate(all_examples):
    ...
    for (feature_index, feature) in enumerate(features):
      ...
      start_prob=softmax(result.start_logits)
      end_prob=softmax(result.end_logits)
        ...
        for end_index in end_indexes:
          ...
          prelim_predictions.append(
              _PrelimPrediction(
                  ...
                  end_index=end_index,
                  start_logit=start_prob[start_index],
                  end_logit=end_prob[end_index]))
      ##Write the outputPrediction to file
      temp_feature={"unique_id": feature.unique_id,
      ...
      #time.sleep(2)
      features_output.append(temp_feature)
      ##
    if FLAGS.version_2_with_negative:
      prelim_predictions.append(
          _PrelimPrediction(
...
## merge same answer in different span and different 
    ...
    def get_score(score_list):
        start_probs=[x[0] for x in score_list]
        end_probs=[x[1] for x in score_list]
        score=(math.log(sum(start_probs))+math.log(sum(end_probs)))
        return score
...
##Write the outputPrediction to file
    #print(features_output[0]["unique_id"])
    all_outputPredictions[example.qas_id] = features_output
    doc_index = example.qas_id
    doc_index += '_'
    doc_index += str(example_index)
    all_outputPredictions[doc_index] = example.doc_tokens
##
  ...
  with tf.gfile.GFile(os.path.join(FLAGS.output_dir, "outputPredictions.json"), "w") as writer:
    writer.write(json.dumps(all_outputPredictions, indent=4) + "\n")
...
##Write features to file
    ...
    with tf.gfile.GFile(json_file, "w") as writer:
        writer.write(json.dumps(features_output, indent=4) + "\n")
...
## read features from file  
    ...
    with tf.gfile.GFile(json_file, "r") as reader:
        ...
        for entry in input_data:
            ...
            #time.sleep(10)
            temp_feature = InputFeatures(
              unique_id=entry["unique_id"],
              example_index=entry["example_index"],
              doc_span_index=None,
              tokens=entry["tokens"],
              token_to_orig_map=entry["token_to_orig_map"],
              token_is_max_context=entry["token_is_max_context"],
              input_ids=None,
              input_mask=None,
              segment_ids=None,
              start_position=None,
              end_position=None,
              is_impossible=False)
            my_eval_features.append(temp_feature)
       # print(len(eval_features))
       # print (eval_features[0].unique_id)
       # time.sleep(2)
